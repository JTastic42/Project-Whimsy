...
import { RUNNING_WORKOUT_TYPES } from '../../utils/constants';
import { calculatePace, parseDurationToMinutes } from '../../utils/paceUtils';
import './RunningForm.css';
...
const RunningForm = ({
  ...
  const [workoutType, setWorkoutType] = useState(initialData?.workoutType || 'Long Run');
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-row">
          <div className="input-group">
            <label htmlFor="run-type">Workout type</label>
            <select
              id="run-type"
              value={workoutType}
              onChange={(e) => setWorkoutType(e.target.value)}
            >
              {RUNNING_WORKOUT_TYPES.map((t) => (
                <option key={t} value={t}>{t}</option>
              ))}
            </select>
          </div>
          <div className="input-group">
            <label htmlFor="run-distance">Distance ({distanceUnit || 'mi'})</label>
            <input
              id="run-distance"
              type="number"
              step="0.01"
          ...
          <div className="input-group">
            <label htmlFor="run-duration">Duration (min or HH:MM)</label>
            <input
              id="run-duration"
              type="text"
              value={duration}
              onChange={(e) => setDuration(e.target.value)}
              placeholder="e.g. 42 or 1:05"
            />
          </div>
        </div>
